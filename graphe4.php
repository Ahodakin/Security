...
for ($mois = 1; $mois <= 12; $mois++) { 
    $info = $db->prepare("SELECT COUNT(*) AS total FROM meurtres_atteintes_physiques INNER JOIN ville ON lieu = ville.id WHERE MONTH(dateEvenement) = $mois AND ((lieu ='$lieu') AND (YEAR(dateEvenement)= '$annee'))");
    $info->execute();
